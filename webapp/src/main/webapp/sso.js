 ...
 *
 * 功能说明：
 * 实现多个应用之间的单点登录（SSO）功能，应用可以部署在不同的域名。容器的退出直接写在头里，避免JS过多加载
 *
 * 版本: 1.0.0.1
  ...
  this._serviceLogin = function(callback, username, password, serviceTicket) {
    ...
    $.ajax({
        ...
        success: function (html) {
            html = $.trim(html).replace(/\t/g, '');
            if(html=="success"){
            	_this._callback(callback,{result: "success", code: "3001", data: html});
			}else{
				_this._callback(callback,{result: "fail", code: "-2", data: "登录失败！"+html});
			}
        },
        error: function (data) {
